/*Задача 1. Разработка через тестирование
        Описание
        Так как мы прошли Test Driven Development, попытаемся разработать минимальную программу с помощью этого подхода. Вам необходимо разработать класс телефонной книги, которая позволяет:

        add - добавить имя с номером. Метод должен возвращать количество контактов после добавления, при этом гарантируется, что не будут добавляться повторяющиеся имена;
        findByNumber - найти имя по номеру без полного перебора;
        findByName - найти номер по имени без полного перебора;
        printAllNames - вывести все имена в алфавитном порядке без необходимости проводить сортировку.
        Реализация
        Класс телефонной книги следует разрабатывать поэтапно, по одному методу за раз. Следует следовать техникам TDD. Таким образом, разработка должна следовать следующим пунктам:

        Создать пустой мавен-проект - done;
        Создать пустой класс PhoneBook - done;
        Создать пустой класс тестов PhoneBookTest - done;
        Сделать коммит с сообщением Initial commit - done;
        Создать пустой метод-заглушку add в классе PhoneBook, который просто возвращает false - done;
        Создать тест(ы) на метод add в классе тестов PhoneBookTest - done;
        Убедиться, что всё компилируется и тесты не проходят. Сделать коммит с сообщением Stub and tests for add - done;
        Реализовать метод add так, чтобы все тесты проходили - done;
        Сделать коммит с сообщением add implemented - done;
        Создать пустой метод-заглушку findByNumber в классе PhoneBook, который просто возвращает null;
        Создать тест(ы) на метод findByNumber в классе тестов PhoneBookTest;
        Убедиться, что всё компилируется и тесты не проходят. Сделать коммит с сообщением Stub and tests for findByNumber;
        Реализовать метод findByNumber так, чтобы все тесты проходили;
        Сделать коммит с сообщением findByNumber implemented;
        ...
        После того как вся функциональность реализована, сделайте пуш в новый репозиторий. Убедитесь, что все тесты проходят и у вас сделаны все требуемые в задании коммиты.
*/




package org.example;

public class Main {
    public static void main(String[] args) {


    }
}